				...
				if (config.chain !== true) {
					...
					chainedSubscribers.push(
						{ 
							...
							argumentArray: [data, topicTree[branch], publisher, onReply, subscriptionIdObject, settings]
						}
					);
				}
			}
 ...
 *			OPTIONAL. defaults to the original publication settings. an option to overwrite the publish options (like silent, persist, chain, ... )
 *		publisher - {object||string}
...
aBubPubSub.prototype.announce = function(topic, data, settings, publisher) {
	...
	if (typeof this.publicationAnnouncements[topic] === 'undefined') {
		this.publicationAnnouncements[topic] = [];
	}
...
aBubPubSub.prototype.__stripSlashes = function(topic) {
	...
	return topic.replace(new RegExp('^[/]+|[/]+$|(/)+/', 'g'), '$1');
};
