 ...
 *		- v0.70 21.03.12
 *			- renamed
 *			- fused client and server (nodeJS) versions
 *			- series of bugfixes
 *		- v0.60	29.10.11			
...
aBubPubSub = function(id) {
	var myScope = this;
	this.id = (typeof id === 'string') ? id : 'aBubPubSub';
	this.VERSION = '0.72';
	this.defaults  = {
		bubble: true,
		persist: false,
		...
		getPersists: false,
		scope: myScope,
		// 0 = none,  1 = console.log all pubs and subs by topic and publisher id
		debugging: 0,			
	...
	for (var branch = 0; branch < topicCount; branch++) {
		for (var subId in this.publicationChannels[topicTree[branch]]) {
			...
			if (branch !== 0 && subscriber.getBubbles === false) {
				...
				// new in 0.72, a fired counter for each subscriber. plays well with the autoUnsubscribe feature
				subscriber.fired++;
 ...
 *			autoUnsubscribe - {number||boolean} defaults to FALSE . if a number is given, the subscriber will auto-unsubscribe once it was fired that many times!
 *		subscriberId - {string}
...
aBubPubSub.prototype.subscribe = function(topic, callback, options, subscriberId) {		
	...
	if (options.forceUniqueSubscriber === true) {
...
aBubPubSub.prototype.__stripSlashes = function(topic) {		
	// this might be more speed-efficient if done without regex ... benchmark needed!
